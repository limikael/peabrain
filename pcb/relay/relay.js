import {declareResistor, declarePinHeader, declareEsp32,
		...
		declareDiode, declareRelay, declareTransistor} from "../../js/pcb/component-declarations.js";

function declareRelayStage(sch, num, input) {
	let k=declareRelay(sch,`K${num}`);

	let r=declareResistor(sch,`R1${num}`,1000);
	let d=declareDiode(sch,`D1${num}`,"1n4148w");
	let q=declareTransistor(sch,`Q${num}`,"MMBT2222A");
	r.connect(q.base,input);
	d.connect(k.a1,k.a2);
	q.emitter.connect("GND");
	k.com.connect("ACL");
	k.a1.connect("5V");
	k.a2.connect(q.collector);
...
export default async function(sch, {variant}) {
	...
	let k1=declareRelayStage(sch,1,esp32.gpio0);
	let k2=declareRelayStage(sch,2,esp32.gpio1);
	let k3=declareRelayStage(sch,3,esp32.gpio3);
	let k4=declareRelayStage(sch,4,esp32.gpio6);

	screw2.pin(1).connect("ACN");
	screw2.pin(2).connect("ACL");

	screw3.pin(1).connect("ACN");
	screw3.pin(2).connect(k1.no);
	screw3.pin(3).connect("ACN");
	screw3.pin(4).connect(k2.no);

	screw4.pin(1).connect("ACN");
	screw4.pin(2).connect(k3.no);
	screw4.pin(3).connect("ACN");
	screw4.pin(4).connect(k4.no);
