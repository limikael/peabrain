...
void CanPlugin::init() {
    canopener_quickjs_init(jsEngine->getContext());
    canopener_quickjs_add_Bus(jsEngine->getContext(),"canBus",&espBus);
    ...
    espBus.messageDispatcher.on([this](cof_t *frame) { handleFrame(frame); });
}

void CanPlugin::handleFrame(cof_t *frame) {
    if (JS_IsUndefined(canMessageFunc))
        return;
...
void CanPlugin::close() {
    espBus.messageDispatcher.off();
    espBus.loopDispatcher.off();
